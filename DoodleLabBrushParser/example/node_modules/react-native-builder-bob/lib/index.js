...
_yargs.default.command('init', 'configure the package to use bob', {}, async () => {
...
}).command('run [script...]', 'run a package.json script', {
  cwd: {
    type: 'string',
  ...
  var _execpath$split$pop, _argv$script;
  ...
  const args = ((_argv$script = argv.script) === null || _argv$script === void 0 ? void 0 : _argv$script.map(s => String(s))) ?? [];
  const cwd = argv.cwd ? _path.default.resolve(process.cwd(), argv.cwd) : process.cwd();
  const scripts = Object.keys(JSON.parse(_fsExtra.default.readFileSync(_path.default.join(cwd, 'package.json'), 'utf-8')).scripts || {});
  ...
  const options = {
    ...
    stdio: 'inherit',
    encoding: 'utf-8',
    shell: _os.default.type() === 'Windows_NT'
  };

  const dir = _path.default.relative(process.cwd(), cwd);

  logger.info(`Running ${_kleur.default.cyan(`${cli} ${args.join(' ')}`.trim())}${dir ? ` at ${_kleur.default.blue(dir)}` : ''}`);

  const result = _crossSpawn.default.sync(cli, args, options);
